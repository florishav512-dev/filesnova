...
/**
 * QrGeneratorPage lets users generate a QR code from text or a URL.
 * Uses ToolSeo + TOOL_SEO_DATA for meta, canonical, JSON-LD, and breadcrumbs.
 */
const QrGeneratorPage: React.FC = () => {
  ...
  // Pull SEO config; provide a safe fallback if the key isn't present yet.
  const SEO_KEY = "/tools/qr-generator";
  const seo =
    TOOL_SEO_DATA?.[SEO_KEY] ?? {
      // Fallbacks — adjust text if you prefer.
      ...
      // BreadcrumbList via ToolSeo props (keep IDs/paths aligned to your router)
      breadcrumbs: [
      ...
      // Optional: extra JSON-LD blocks if your ToolSeo supports `additionalJsonLd`
      additionalJsonLd: [
        {
  ...
  return (
    <>
      {/* SEO (title, meta, canonical, BreadcrumbList JSON-LD, etc.) */}
      <ToolSeo {...seo} />
