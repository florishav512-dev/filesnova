...
/**
 * ImageToPdfPage allows users to combine multiple image files (JPG, PNG, etc.) into a single PDF.
 * The page follows the modern conversion layout with fixed header, gradient sections and progress display.
 */
const ImageToPdfPage: React.FC = () => {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selected) {
      ...
      const mapped = images.map((file) => ({ id: `${file.name}-${Date.now()}-${Math.random()}`, file, status: 'ready' as const }));
  ...
  return (
    <>
      <Helmet>
        <title>Images to PDF Converter – Combine Images into PDF | FilesNova</title>
        <meta
          name="description"
          content="Combine JPG, PNG and other images into a single PDF file securely with our free online Images to PDF converter."
        />
        <link rel="canonical" href="https://filesnova.com/tools/image-to-pdf" />
      </Helmet>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center h-20 space-x-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 font-medium">Images to PDF Converter</p>
            </div>
          </div>
        </div>
      </header>
