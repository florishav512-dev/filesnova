...
const PngToPdfPage: React.FC = () => {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selected) {
      ...
      const mapped = pngs.map((file) => ({ id: `${file.name}-${Date.now()}-${Math.random()}`, file, status: 'ready' as const }));
  ...
  return (
    <>
      <Helmet>
        <title>PNG to PDF Converter – Convert PNG Images to PDF | FilesNova</title>
        <meta
          name="description"
          content="Create a PDF file from your PNG images quickly and securely with our free online PNG to PDF converter."
        />
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 py-10">
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 mb-8">
          <div className="p-8">
            ...
            <input ref={inputRef} type="file" accept="image/png" multiple className="hidden" onChange={handleFileSelect} />
          </div>
          {files.length > 0 && (
