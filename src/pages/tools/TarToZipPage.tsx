...
const TarToZipPage: React.FC = () => {
  ...
  // Parse a TAR buffer and repackage files into a ZIP blob
  const parseTarToZip = async (arrayBuffer: ArrayBuffer) => {
    ...
    while (offset + 512 <= totalSize) {
      ...
      const name = text(header.subarray(0, 100)).trim();
      if (!name) break;
      ...
      // Typeflag (0 or '0' or undefined -> regular file)
      const typeflag = header[156]; // ASCII code
      const isRegular =
        typeflag === 0 || typeflag === '0'.charCodeAt(0) || typeflag === undefined;

      const dataStart = offset + 512;
      const dataEnd = dataStart + size;

      if (isRegular && size > 0 && dataEnd <= totalSize) {
        const fileBlob = new Blob([arrayBuffer.slice(dataStart, dataEnd)]);
        zip.file(name, fileBlob);
      }

      // Advance to next 512-byte boundary after file contents
          ...
          <div className="relative bg-gradient-to-br from-blue-50 to-indigo-100 rounded-3xl p-8 mb-8 overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-gray-700 text-lg leading-relaxed">
                Convert TAR archives into ZIP format quickly and securely.
              </p>
