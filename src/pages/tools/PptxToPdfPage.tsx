...
/** ---------- SEO CONSTANTS (exported) ---------- */
export const TOOLSEO = {
  title: 'PPTX to PDF – Fast & Free Online Converter | FilesNova',
  description:
    'Instantly convert PPTX slides to PDF (text-only extraction). 100% free, no signup, no watermarks—fast, secure, and private on FilesNova.',
...
export const TOOL_SEO_DATA = {
  breadcrumb: [
    ...
    { name: 'PPTX to PDF', url: TOOLSEO.canonical },
  ],
};
...
const PptxToPdfPage: React.FC = () => {
    ...
    try {
      ...
      for (let i = 0; i < slides.length; i++) {
        ...
        for (let j = 0; j < lines.length; j++) {
          const ln = lines[j];
          if (y < margin) {
            page.drawText('[Slide continues on next page]', {
              x: margin,
              y: margin,
              size: 10,
              font,
  ...
  return (
    <>
      <Helmet>
        <title>{TOOLSEO.title}</title>
        <meta name="description" content={TOOLSEO.description} />
        <link rel="canonical" href={TOOLSEO.canonical} />
      </Helmet>
      ...
      <JsonLd
        data={{
          '@context': 'https://schema.org',
          '@type': 'BreadcrumbList',
          itemListElement: TOOL_SEO_DATA.breadcrumb.map((b, idx) => ({
            '@type': 'ListItem',
            position: idx + 1,
            name: b.name,
            item: b.url,
          ...
          <div className="relative bg-gradient-to-br from-blue-50 to-indigo-100 rounded-3xl p-8 mb-8 overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-gray-700 text-lg leading-relaxed">
                Extract text from your PowerPoint presentations and export them as PDFs.
              </p>
