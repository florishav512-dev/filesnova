...
/**
 * ExtractZipPage unpacks the contents of a ZIP file directly in the browser.
 * Users can download individual files or repackage all extracted files into a
 * new ZIP archive. Directory structures inside the archive are flattened.
 */
const ExtractZipPage: React.FC = () => {
  ...
  // create a new ZIP from extracted files for convenience
  const downloadAll = async () => {
    const zip = new JSZip();
    for (const ef of extractedFiles) {
      const resp = await fetch(ef.url);
      const blob = await resp.blob();
      zip.file(ef.name, blob);
    }
  ...
  return (
    <>
      ...
      <Helmet>
        ...
        <meta
          name="description"
          content="Unpack ZIP archives in your browser. Fast, free, and secure—download files individually or repackage everything into a new ZIP. No signup."
        />
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 relative overflow-hidden pt-24">
        ...
        <header className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-xl shadow-lg border border-white/20">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 py-10">
          <div className="relative bg-gradient-to-br from-blue-50 to-indigo-100 rounded-3xl p-8 mb-8 overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-gray-700 text-lg leading-relaxed">Unpack the contents of a ZIP file and download the files.</p>
            ...
            <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-8 mb-8">
              ...
              <ul className="space-y-2 max-h-60 overflow-y-auto">
                {extractedFiles.map((ef) => (
                  <li key={ef.url} className="flex justify-between items-center bg-white/60 backdrop-blur-sm p-2 rounded-lg">
                    <span className="text-sm break-all">{ef.name}</span>
                    <a
                      href={ef.url}
                      download={ef.name}
